...
local function bnOnClick(self, info, btn)
	if btn == "LeftButton" and IsShiftKeyDown() then
		-- BN invite / 戰網邀請
		if info[5] == BNET_CLIENT_WOW and info[11] == GetRealmName() then
	...
	for i = 1, num do
		...
		if accountInfo then
			...
			if isOnline and gameID then
				...
				local realmDisplayName = gameAccountInfo.realmDisplayName or ""
				...
				-- Check classic or retail / 區分不同版本的遊戲
				...
				tinsert(bnetTable, {i, accountName, battleTag, charName, client, faction, status, class, level, infoText, realmDisplayName, isMobile})
			end
		end
	end
...
local function OnEnter(self)
	...
	if numBNetOnline > 0 then
		...
		for i = 1, #bnetTable do
			...
			if F.Multicheck(info[5], BNET_CLIENT_WOW, CLIENT_WOW_DIFF) then
				local zonec
				...
				if isShiftKeyDown then
					...
					tooltip:AddLine(BNet_GetClientEmbeddedAtlas(info[5], 12).." "..G.OptionColor..info[3].."|r"..info[7], F.Hex(.65, .65, .65)..info[10])
				else
					tooltip:AddLine(BNet_GetClientEmbeddedAtlas(info[5], 12).." "..G.OptionColor..info[4].."|r"..info[7], F.Hex(.65, .65, .65)..info[10])
				end
			end
